...
void fhe(int signal_len) {
    ...
    std::vector<integer> coeffs_B = {1, 2, 3}; 
    std::vector<integer> coeffs_A = {1, -1, -2}; 

    std::vector<Ciphertext> output(signal_len);
    int filter_len = coeffs_B.size();  
    ...
    for (int i = 0; i < signal_len; i++) {
        Ciphertext feedforward = encrypt(0);
        Ciphertext feedback = encrypt(0);

        for (int j = 0; j < filter_len; j++) {
            if ((i - j) >= 0) {
                feedforward += signal[i - j] * coeffs_B[j];
            }
        }

        for (int k = 1; k < filter_len; k++) {
            if ((i - k) >= 0) {
                feedback += output[i - k] * coeffs_A[k]; 
            }
        }

        output[i] = feedforward + feedback;  
